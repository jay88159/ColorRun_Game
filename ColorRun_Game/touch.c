    ...
    if((g_sTouchX < g_sTouchMin) || (g_sTouchY < g_sTouchMin))
    {
        ...
        if(g_cState != 0x00)
        {
            ...
            if(g_cState == 0x80)
            {
                ...
                if(g_pfnTSHandler)
                {
                	/*System_printf("*********g_pfnTSHandler*********00\n");
                	 System_flush();*/
                	        		    /* SysMin will only print to the console when you call flush or exit */
                	//    System_flush();
                    //
    ...
    else
    {
        ...
        else
        {
            ...
            if(g_cIndex == -2)
            {
                ...
                if(g_pfnTSHandler)
                {
                	/*System_printf("*********g_pfnTSHandler********11*\n");
                	System_flush();*/
                	//
            ...
            else
            {
                ...
                if(g_pfnTSHandler)
                {
                	/*System_printf("*********g_pfnTSHandler********22*\n");
                	System_flush();*/
                	//
...
{
	  System_printf("ADC 0 sequence 3 init");
   ...
   /* if((HWREG(TIMER1_BASE + TIMER_O_CTL) & TIMER_CTL_TAEN) == 0)
    {*/
    	System_printf("ADC 320 sequence 3 init");
